...
public class Search {
  ...
  public JSONResponse checkURL(String query, HttpServletRequest request) {
    synchronized (urlsBeingChecked) {
      ...
      if (urlsBeingChecked.containsKey((String) query) == false) {
        ...
        if (urlBeingChecked.getCreationDate() + (catalogCheckerTimeValiditySec * 1000) < DateFunctions
  ...
  private JSONObject makeJSONFromSearchQuery(String query, HttpServletRequest request)
    ...
    try {
      ...
      if (numFound > maxAmountOfDataSetsInJSON) {
        throw new JSONResponse.JSONResponseException(
            "Too many results, maximum of " + maxAmountOfDataSetsInJSON + " datasets allowed.", 200);
      } else {
          ...
          try {
            ...
            for (int i = 0; i < flat.length(); i++) {
              ...
              try {
                ...
                totalFileSize = totalFileSize + Long.parseLong(fileSize);
              } catch (Exception e) {
              }
