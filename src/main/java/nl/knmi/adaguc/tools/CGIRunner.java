...
public class CGIRunner {
	...
	/**
	 * Runs a CGI program as executable on the system. Emulates the behavior of
	 * scripts in a traditional cgi-bin directory of apache http server.
	 * 
	 * @param commands             A string array with the executable and the
	 *                             arguments to this executable
	 * @param environmentVariables The environment variables used for this run
	 * @param directory            The location to start the program, can be null or
	 *                             "" if not needed
	 * @param response             Can be null, when given the content-type for the
	 *                             response will be set.
	 * @param outputStream         A standard byte output stream in which the data
	 *                             of stdout is captured. Can for example be
		...
		class StdoutPrinter implements ProcessRunner.StatusPrinterInterface {
			...
			public void _print(byte[] message, int bytesRead) throws IOException {
				...
				if (headersSent == false) {
					...
					if (headersSent == true) {
						response.setHeader("ADAGUCSERVICESINFO", (new JSONObject().put("pid", pid).toString()));
						...
						for (int h = 0; h < headerItem.length; h++) {
							...
							if (headerKVP.length == 2) {
								...
								if (headerKVP[0].trim().equalsIgnoreCase("Content-Type")) {
									...
									// Debug.println("Setting header
									// ["+headerKVP[0].trim()+"]=["+headerKVP[1].trim()+"]");
		...
		if (response != null) {
			if (processRunner.exitValue() != 0 && processRunner.exitValue() != 1) {
				...
				if (processRunner.exitValue() == 143) {
					msg += " (Timeout set to " + timeOutMs + " ms, process took " + ((stopTimeInMillis - startTimeInMillis))
							+ " ms)";
				}
